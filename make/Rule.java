...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("Error ERROR error "
    ...
    private void checkFinishedDependencies() {
        for (int x : _depends.successors(_vertex)) {
            Rule nextRule = _depends.getLabel(x);
            if (nextRule.isUnfinished()) {
                error("Error: There are unfinished dependencies.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        return getTime() < _maker.getCurrentTime();
    }
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            if (_commands.isEmpty() && getTime() == null) {

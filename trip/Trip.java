...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Location start = _map.getLabel(from);
        String startString = start.toString();
        String previousString = startString;
        int previousInt = from;
        segment.remove(0);
        while (segment.size() > 0) {
            int next = segment.get(0);
            Location nextLocation = _map.getLabel(next);
            String nextLocationString = nextLocation.toString();
            Road roadBetween = _map.getLabel(previousInt, next);
            String roadName = roadBetween.toString();
            Direction roadDirection = roadBetween.direction();
            String directionName = roadDirection.fullName();
            Double distance = roadBetween.length();
            int scale = (int) Math.pow(10, 1);
            previousInt = next;
            int count = 1;
            if (segment.size() > 1) {
                int ad = segment.get(count);
                Road roadBetweenNext = _map.getLabel(next, ad);
                while (roadBetween.toString().equals(roadBetweenNext.toString())
                        && (roadBetween.direction().fullName().equals(
                                roadBetweenNext.direction().fullName()))) {
                    distance += roadBetweenNext.length();
                    previousInt = segment.get(count);
                    nextLocationString = _map.getLabel(previousInt).toString();
                    segment.remove(count);
                    if (segment.size() > 1) {
                        roadBetweenNext =
                                _map.getLabel(previousInt, segment.get(count));
                    } else {
                        break;
                    }
                }
            ...
            String result = seq + ". Take " + roadName + " "
                    + directionName + " for " + distance + " miles.";
            if (segment.size() == 1) {
                result = seq + ". Take " + roadName + " "
                        + directionName + " for " + distance
                        + " miles to " + nextLocationString + ".";
            }
    ...
    private void addRoad(String from, String name, double length,
        ...
        if (v0 == null) {
            ...
            _map.add(_sites.get(from),
                    _sites.get(to), new Road(name, dir, length));
            _map.add(_sites.get(to),
                    _sites.get(from), new Road(name, dir.reverse(), length));
        }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            Road ourRoad = _map.getLabel(u, v);
            return ourRoad.length();
        }
